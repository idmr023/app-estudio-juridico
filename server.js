...
app.get('/api/casos/:caso_id', async (req, res) => {
    const { caso_id } = req.params;

    try {
        const [
            [casoData], // La consulta de casos devuelve un array, lo desestructuramos para obtener el primer objeto
            eventos,
            documentos,
            mensajes
        ] = await Promise.all([
          db.promise().query(`
            ...
            db.promise().query('SELECT * FROM eventos_caso WHERE caso_id = ? ORDER BY fecha_evento DESC', [caso_id]),

            db.promise().query('SELECT * FROM documentos_caso WHERE caso_id = ? ORDER BY fecha_subida DESC', [caso_id]),

            db.promise().query(`
                SELECT m.*, u.usr_nom AS nombre_remitente
                FROM mensajes_caso AS m
                JOIN usuario AS u ON m.remitente_dni = u.usr_dni
                WHERE m.caso_id = ? 
